    ...
    IWebHostEnvironment webHostEnvironment)
{
    ...
    public async Task<string> RenderDashboardHtmlAsync(string dashboardId, string layoutConfigJson)
    {
        ...
        {
            _logger.LogError(ex, "SSR: Failed to render dashboard {DashboardId}. LayoutConfig: {LayoutConfig}", dashboardId, layoutConfigJson?.Substring(0, Math.Min(500, layoutConfigJson?.Length ?? 0)));
            throw new InvalidOperationException($"Failed to render dashboard: {ex.Message}", ex);
        }
    }
    ...
    private LayoutConfig ParseLayout(string json)
    {
        ...
        {
            // Use default color scheme if not provided
            colorScheme = new ColorSchemeConfig(
                Name: "Default",
                Variant: null,
        ...
        if (root.TryGetProperty("widgets", out var widgetsArr) && widgetsArr.ValueKind == JsonValueKind.Array)
        {
            ...
            foreach (var w in widgetsArr.EnumerateArray())
            {
                ...
                // Skip widgets without required properties
...
";
    ...
    {
        _logger.LogInformation("SSR: Generating canvas HTML for {WidgetCount} widgets", layout.Widgets.Count);
