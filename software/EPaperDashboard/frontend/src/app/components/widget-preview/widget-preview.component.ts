...
import { AiTextWidgetComponent } from '../widgets/ai-text-widget.component';
import {
  WidgetConfig,
  ColorScheme,
  HeaderConfig,
...
@Component({
  ...
  template: `
    <div class="widget-preview">
      ...
      @if (widget.type === 'ai-text') {
        <app-widget-ai-text [widget]="widget" [colorScheme]="colorScheme" [designerSettings]="designerSettings"></app-widget-ai-text>
      }
      }
    </div>
  `,
...
export class WidgetPreviewComponent {
  ...
  private static readonly WIDGET_META: Record<string, { icon: string; label: string }> = {
    ...
    'ai-text':          { icon: 'fa-robot',         label: 'AI Text' },
  };
