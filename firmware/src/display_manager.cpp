...
void DisplayManager::showConfirmationPin(const String& pin)
{
  _logger.println("Displaying confirmation PIN...");
  ...
  do
  {
    ...
    _display.getTextBounds("Confirm Pairing", 0, 0, &tbx, &tby, &tbw, &tbh);
    _display.setCursor((DisplayConst::Width - tbw) / 2, 160);
    _display.print("Confirm Pairing");

    _display.setFont(&FreeSans12pt7b);
    const char* instruction = "Verify this PIN matches your dashboard:";
    _display.getTextBounds(instruction, 0, 0, &tbx, &tby, &tbw, &tbh);
    _display.setCursor((DisplayConst::Width - tbw) / 2, 220);
    _display.print(instruction);

    _display.setFont(&FreeSansBold18pt7b);
    _display.setTextColor(GxEPD_RED);
    _display.setTextSize(2);
    _display.getTextBounds(pin.c_str(), 0, 0, &tbx, &tby, &tbw, &tbh);
    _display.setCursor((DisplayConst::Width - tbw) / 2, 310);
    _display.print(pin);
    _display.setTextSize(1);

    _display.setFont(&FreeSans12pt7b);
    _display.setTextColor(GxEPD_BLACK);
    const char* footer = "Then click 'Confirm' in your browser.";
    _display.getTextBounds(footer, 0, 0, &tbx, &tby, &tbw, &tbh);
    _display.setCursor((DisplayConst::Width - tbw) / 2, 400);
    _display.print(footer);

  } while (_display.nextPage());

  _logger.println("Confirmation PIN displayed");
}
